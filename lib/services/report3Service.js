(function() {
	...
	window.services.report3Service = function(logService) {
		var private = {
			debug: false
		};
		...
		private.getPersonRow = function(courses, person) {
			if (!courses || typeof courses !== 'object' || courses.length < 1 
				...
				logService.safeLog('getPersonRow', courses.length + ' ' + person.name);
				logService.safeLog('getPersonRow courses', JSON.stringify(courses));
				logService.safeLog('getPersonRow person', JSON.stringify(person));
				...
				window.utils.fastLoop(courses, function(course) {
					...
					if (currentLos && currentLos.length > 0) {
						...
						window.utils.fastLoop(course.los, function(lo) {
		...
		private.aggregateSegmentByStore = function(colGroup, rowGroup, model) {
			if (!colGroup || typeof colGroup !== 'object' || !colGroup.hasOwnProperty('id') || !colGroup.id
				...
				window.utils.fastLoop(peopleRows, function(personRow) {
					// include row in calculation only if personRow.show is true
					if (personRow.show) {
						var currentLos = [];
						window.utils.fastLoop(Object.keys(personRow), function(k) {
							if (personRow[k].hasOwnProperty('parentId') && personRow[k].parentId === colGroup.id) {
								currentLos.push(personRow[k]);
							}
						});
						logService.safeLog('aggregateSegmentByStore: personRow ' + personRow.id + ' currentLos', currentLos)

						var aggregatedLos = 0, losCount = currentLos.length, naLosCount = 0;

						window.utils.fastLoop(currentLos, function(lo) {
							...
							if (colChild.calculate) {
								...
								logService.safeLog('aggregateSegmentByStore [' + colChild.id + ' is hidden]', colChild);
								losCount = losCount > 0 ? --losCount : 0;
							}
				...
				logService.safeLog('aggregateSegmentByStore peopleCount: ' + peopleRowsCount + ' naCount: ' + naCount, aggregated)
				var finalValue;
				if (naCount !== peopleRows.length) {
					finalValue = peopleRowsCount > 0 ? Math.round(aggregated / peopleRowsCount * 100) / 100 : 0;
				} else {
		...
		private.aggregateLoByStore = function(colChild, rowGroup, model) {
			...
			window.utils.fastLoop(peopleRows, function(personRow) {
				...
				if (personRow.show) {
					...
					logService.safeLog('aggregateLoByStore: personRow ' + personRow.id + ' lo ' + colChild.id + ' ', currentLo)
					...
					if (currentLo && currentLo.realValue > -1) {
						...
						if (lookupLo.calculate) {
							...
							logService.safeLog('aggregateSegmentByStore [' + lookupLo.id + ' is hidden]', lookupLo);
						}
		...
		var recalculate = function(model, data) {
			logService.safeLog('recalculate - WORKING ON IMPLEMENTATION');
			...
			window.utils.fastLoop(rowGroups, function(rowGroup) {
				...
				if (rowGroup.show) {
					...
					window.utils.fastLoop(colGroups, function(colGroup) {
						...
						if (!topLevelColumn && colGroup.show || (topLevelColumn && topLevelColumn.id === colGroup.id)) {
							...
							window.utils.fastLoop(colChildren, function(colChild) {
					...
					window.utils.fastLoop(rowChildren, function(personRow) {
							...
							window.utils.fastLoop(colGroups, function(colGroup) {
								...
								if (!topLevelColumn && colGroup.show || (topLevelColumn && topLevelColumn.id === colGroup.id)) {
									...
									window.utils.fastLoop(currentLos, function(lo) {
										...
										// check if col.calculate is true
										if (colChild.calculate) {
											logService.safeLog('recalculate: personRow.id ' + personRow.id + ' ' + colGroup.id + ' [' + lo.id + ']', lo);
											if (lo.realValue > -1) {
												aggregatedLos += lo.realValue === 2 ? 1 : 0;
											} else {
												naLosCount++;
											...
											logService.safeLog('recalculate: personRow.id ' + personRow.id + ' ' + colGroup.id + ' [' + lo.id + ' is hidden]', lo);
											naLosCount++;
											losCount = losCount > 0 ? --losCount : 0;
										}
									});
									...
									if (naLosCount !== currentLos.length) {
										...
										logService.safeLog('recalculate: personRow.id ' + personRow.id + ' ' + colGroup.id + ' personCourseCell.value', personCourseCell.value);
									} else {
										naCoursesCount++;
										coursesCount--;
		...
		var getModel = function(data) {
            ...
            if (!data.stores[0].people[0].los[0].segmentId) {
                logService.safeLog('Adding segmentId to people learning objects');

				window.utils.fastLoop(data.stores, function(store) {
                    window.utils.fastLoop(store.people, function(person) {
                        window.utils.fastLoop(person.los, function(personLo) {
                            var itemLo;
                            window.utils.fastLoop (data.segments, function(segm) {
                ...
                logService.safeLog('Data with segmentId on people learning object', data);
            }
			...
			window.utils.fastLoop(data.segments, function(course) {
				...
				// add child columns
				window.utils.fastLoop(course.los, function(section) {
					// child cell
					var colChild = {
						isChild: true,
						parentId: course.id,
			...
			window.utils.fastLoop(data.stores, function(store) {
				...
				logService.safeLog('peopleByStore', peopleByStore);
				...
				window.utils.fastLoop(data.segments, function(course) {
					...
					window.utils.fastLoop(course.los, function(courseLo) {
						var childCell = {
							isChild: true,
							parentId: course.id,
							id: courseLo.id,
				...
				window.utils.fastLoop(peopleByStore, function(person) {
			...
			logService.safeLog('model', JSON.stringify(model));
