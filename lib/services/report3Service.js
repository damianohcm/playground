	...
	window.services.report3Service = function() {
		...
		private.getPersonRow = function(courses, person) {
			if (!courses || typeof courses !== 'object' || courses.length < 1 
				...
				// show by default (but will be hidden initially as store row will have isCollapsed true)
				person.show = person.hasOwnProperty('show') ? person.show : true;
		...
		var getModel = function(data) {
			...
			_.each(data.segments, function(course) {
				...
				_.each(course.los, function(section) {
					section.show = section.hasOwnProperty('show') ? section.show : false; // for debugging you can change to true
