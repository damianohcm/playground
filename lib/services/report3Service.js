		...
		var recalculate = function(model, data) {
			...
			_.each(rowGroups, function(rowGroup) {
				...
				_.each(colGroups, function(colGroup) {
					...
					// if group column is visible and we are not in detail view (topLevelColumn will be undefined)
					if (!topLevelColumn && colGroup.show || topLevelColumn.id === colGroup.id) {
