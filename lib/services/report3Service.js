(function() {
	...
	var service = function() {
		...
		var getPersonSegmentCellCss = function(cell) {
			if (isNaN(cell.value)) {
				return 'col-group na';
		...
		var getPersonRow = function(courses, person) {
			...
			_.each(courses, function(course) {
				...
				if (currentLos && currentLos.length > 0) {
					...
					cell.css = getPersonSegmentCellCss(cell);
					...
					_.each(currentLos, function(lo) {
						// child cell
						row[course.id + '_' + lo.id] = personLoCells[lo.value];
					});
				} else {
		 ...
		 * @method 
		 */
		var aggregateSegmentByStore = function(course, store) {
		...
		var getModel = function(data) {
			...
			_.each(data.stores, function(store) {
				...
				_.each(data.segments, function(course) {
					...
					cell.value = aggregateSegmentByStore(course, store);
					if (cell.value === 'N/A') {
						cell.suffix = '';
						naCoursesCount++;
						coursesCount = coursesCount > 0 ? --coursesCount : 0;
					...
					// store cell into row using course.id as the row property
					rowGroup[course.id] = cell;

					// _.each(course.los, function(courseLo) {

					// 	var aggregatedLos = 0, peopleCount = peopleByStore.length;
					// 	_.each(peopleByStore, function(person) {
					// 		var currentLo = _.filter(person.los, function(personLo) {
					// 			return personLo.id = courseLo.id;
					// 		});

					// 		if (currentLo && currentLo.length > 0) {

					// 			aggregatedLos += currentLo.value === 2 ? 1 : 0;

					// 		} else {
					// 			aggregatedLos += 100; // 'N/A' should count as 100% compete
					// 		}
					// 	});

					// 	var peopleAggregated = peopleCount > 0 ? Math.round(aggregatedLos / peopleCount * 100) : 0;

					// 	// child cell
					// 	rowGroup[course.id + '_' + courseLo.id] = {
					// 		value: peopleAggregated
					// 	};
					// });
				});
