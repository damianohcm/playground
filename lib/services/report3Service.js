	...
	window.services.report3Service = function() {
				...
				_.each(courses, function(course) {
					...
					if (currentLos && currentLos.length > 0) {
						...
						_.each(course.los, function(lo) {
							...
							// child cell
							var childCell = private.personLoCells[personLo.value];
							childCell.parentId = course.id;
							childCell.id = lo.id;
							row[course.id + '_' + lo.id] = childCell;
						});
					} else {
						cell.isNA = true;
						cell.value = 'N/A';
						...
						_.each(course.los, function(lo) {
							// child cell
							var childCell = private.personLoCells[-1];
							childCell.parentId = course.id;
							childCell.id = lo.id;
							// child cell
							row[course.id + '_' + lo.id] = childCell;
		...
		private.aggregateLoByStore = function(colChild, rowGroup) {
			...
			_.each(peopleRows, function(personRow) {
				...
				//console.log('personRow ' + person.name + ' lo ' + colChild.id + ' ', currentLo)

				if (currentLo && currentLo.realValue > -1) {
					aggregated += currentLo.realValue === 2 ? 1 : 0;
				} else {
					// if person does not have learnig object, reduce peopleRowsCount and increase naCount
					peopleRowsCount = peopleRowsCount > 0 ? --peopleRowsCount : 0;
					naCount++;
				}
			});
		...
		var recalculate = function(model, data) {
			...
			// 	// 	var groupCell = rowGroup[colGroup.id];
			...
			// 	// 		var childCell = rowGroup[colGroup.id + '_' + colChild.id];

			// 	// 		/*var childCell = {
			// 	// 			isChild: true,
			// 	// 			parentId: colGroup.id,
			// 	// 			id: colChild.id,
			// 	// 			value: private.aggregateLoByStore(colChild, store)
			// 	// 		};*/
			...
			// 	// 	console.log('Need to calculate personRow', personRow);
			// 	// });
			...
			// 				_.each(currentLos, function(lo) {
			// 					console.log('getPersonRow ' + personRow.id + ' ' + colGroup.id + ' [' + lo.id + ']', lo);
			// 					if (lo.realValue > -1) {
			// 						aggregatedLos += lo.realValue === 2 ? 1 : 0;
			// 					}
			// 				});
			...
			// 				var tempAggr = losCount > 0 ? Math.round(aggregatedLos / losCount * 100) : 0;
			// 				console.log('getPersonRow ' + personRow.id + ' ' + colGroup.id + ' tempAggr', tempAggr);

			// 				aggregatedPerson += tempAggr;
			// 			}
			// 		});
