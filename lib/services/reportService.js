	...
	var service = function() {
		...
		var getPersonRow = function(courses, person) {
			...
			_.each(courses, function(course) {
				...
				row.summary.value = sectionsCount > 0 ? Math.round(aggregatedSections / sectionsCount * 100) : 0;
		...
		var getModel = function(data) {
			...
			_.each(data.stores.rows, function(store) {
				...
				// // loop through the people for this store and create a row for each person
				...
				// 		var personCell = getPersonRow(course, person);


				// 		personRow[course.id] = personCell;
				// 	});
				...
				//row.summary.value = storeAggregated > 0 ? coursesCount / storeAggregated * 10000 : 0;
				...
				// the row (horizontal) percentage for all the 
				row.summary.value = storeAggregated; //storesCount > 0 ? storeAggregated / storesCount * 100 : 0;
