(function() {
    ...
    window.controllers.report4Controller = function($scope, utilsService, undoServiceFactory, dataService, reportService, $timeout, $interval) {
		$scope.title = '';//Report4 Controller';

		$scope.undoService = undoServiceFactory.getService('report4Controller');

		$scope.undoLastAction = function() {
			var action = $scope.undoService.undoLastAction();
		...
		$scope.exportToCsv = function() {
			...
			utilsService.safeLog('Report4Controller.exportToCsv', csv);

			if (navigator.msSaveBlob) { // IE10
				utilsService.safeLog('Report4Controller.exportToCsv INTERNET EXPLORER');
				...
				utilsService.safeLog('Report4Controller.exportToCsv HTML5');
			...
			// } else {
			// 	utilsService.safeLog('Report4Controller.exportToCsv OTHERS');
		...
		var onDataError = function(err) {
			utilsService.safeLog('Report4Controller.onDataError', err);
			$scope.error = 'Could not fetch data';
		};

		var onDataComplete  = function(data) {
			utilsService.safeLog('Report4Controller.onDataComplete', data);
			$scope.data = data;
		...
		var fileName = 'report4.json?' + Math.random();
		//var fileName = 'report4-generated1.json?' + Math.random();
		//var fileName = 'report4-generated2.json?' + Math.random();
