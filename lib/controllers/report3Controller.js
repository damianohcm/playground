(function() {
	...
	// create controller
	var Report3Controller = function($scope, logService, undoServiceFactory, dataService, reportService, $timeout, $interval) {
		...
		$scope.exportToCsv = function() {
			var fileName = 'Summary.csv',
				mimeType = '',
				csv = window.utils.getCsv($scope.model);
			console.log('Report3Controller.exportToCsv', csv);

			if (navigator.msSaveBlob) { // IE10
				console.log('Report3Controller.exportToCsv INTERNET EXPLORER');
				...
				console.log('Report3Controller.exportToCsv HTML5');
			...
			// } else {
			// 	console.log('Report3Controller.exportToCsv OTHERS');
		...
		var onDataError = function(err) {
			logService.safeLog('Report3Controller.onDataError', err);
			$scope.error = 'Could not fetch data';
		};

		var onDataComplete  = function(data) {
			logService.safeLog('Report3Controller.onDataComplete', data);
			$scope.data = data;
		...
		$scope.toggleChildRows = function(model, row) {
			logService.safeLog('toggleChildRows', row.children.length);
		...
		$scope.showAllColumns = function() {
			logService.safeLog('showAllColumns');
			if ($scope.model) {
				...
				// update values
				//logService.safeLog('WARNING: code commented out');
				$scope.recalculate();
			}
		};
		...
		$scope.hideCol = function(col) {
			...
			col.show = false;
			if (col.isChild) {
				logService.safeLog('hideCol child column. Calculate is', col.calculate);
			}
