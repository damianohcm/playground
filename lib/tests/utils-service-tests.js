...
(function(){
	...
	requirejs(['../services/utilsService.js', '../services/report3Service.js'], function() {
		mocha.setup('bdd');

		var utilsService = window.services.utilsService()
			, report3Service = window.services.report3Service(utilsService);
		...
		describe('utilsService', function() {
			...
			it('report3Service: should not be undefined', function(done) {
			...
			it('utilsService.fastLoop: should not throw', function(done) {
				...
				var fn = function () { 
					try {
						utilsService.fastLoop([1,2,3], (item) => {
